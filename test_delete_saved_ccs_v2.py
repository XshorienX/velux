...
class TestRunner:
    ...
    def insert_saved_cc_sqlite(self, user_id, card_data):
        """Insert a saved CC directly into SQLite database"""
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            
            # Generate a unique ID for the saved CC
            ...
            cursor.execute(
                "INSERT INTO docs (col, id, doc) VALUES (?, ?, ?)",
                ("saved_ccs", cc_id, doc_str)
            )
            conn.commit()
            conn.close()
    ...
    def test_insert_test_data(self):
        ...
        try:
            ...
            for i in range(3):
                ...
                cc_id = self.insert_saved_cc_sqlite(self.user1_id, card_data)
                if cc_id:
                    self.user1_hits.append(cc_id)
                    self.log(f"Inserted saved CC for user1: {cc_id}")
            ...
            for i in range(2):
                ...
                cc_id = self.insert_saved_cc_sqlite(self.user2_id, card_data)
                if cc_id:
                    self.user2_hits.append(cc_id)
                    self.log(f"Inserted saved CC for user2: {cc_id}")
            
            self.assert_equal(len(self.user1_hits), 3, "User1 has 3 saved CCs inserted")
            self.assert_equal(len(self.user2_hits), 2, "User2 has 2 saved CCs inserted")
